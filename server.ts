...
app.post("/api/chat", async (req: Request, res: Response): Promise<void> => {
  ...
  const studentInput = (req.body?.studentInput || req.body?.message || req.body?.userMessage || req.body?.text || (lastUserMsg ? lastUserMsg.text : "") || "").trim();
...
- "chineseTranslation": Friendly, encouraging Chinese translation of what you just said. Keep it casual and warm. Use Chinese only for this field.`;
    ...
    if (isQuotaError) {
      console.warn("[API WARNING] Gemini API quota limit met, starting local conversational recovery system for coach:", coach.name);
    } else {
